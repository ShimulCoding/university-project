...
import { listBudgetRequests } from "@/lib/api/internal";
...
export default async function EventBudgetRequestsPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const requests = await listBudgetRequests({ eventId: event.id }).catch(() => []);
  ...
  return (
    <div className="flex flex-col gap-8 pb-16">
      <section className="relative overflow-hidden rounded-3xl border border-border/20 bg-background/50 shadow-xl backdrop-blur-3xl px-8 py-8">
        <h1 className="text-3xl font-black tracking-tight">Budget Requests</h1>
        <p className="mt-2 text-muted-foreground text-lg font-light">Funding proposals for {event.title}.</p>
      ...
      <Card className="border-border/40 shadow-xl bg-background/40 backdrop-blur-xl overflow-hidden">
        <CardHeader className="border-b border-border/30 bg-muted/10"><CardTitle>Budget Requests</CardTitle></CardHeader>
        <CardContent className="p-0">
          {requests.length === 0 ? (
            <div className="p-12"><StatePanel icon={SearchSlash} tone="empty" title="No budget requests" description="No funding proposals have been submitted for this event." /></div>
          ) : (
