...
import { listIncomeRecords } from "@/lib/api/internal";
...
export default async function EventIncomeRecordsPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const records = await listIncomeRecords({ eventId: event.id }).catch(() => []);
  ...
  return (
    <div className="flex flex-col gap-8 pb-16">
      <section className="relative overflow-hidden rounded-3xl border border-border/20 bg-background/50 shadow-xl backdrop-blur-3xl px-8 py-8">
        <h1 className="text-3xl font-black tracking-tight">Income Records</h1>
        <p className="mt-2 text-muted-foreground text-lg font-light">Income tracking for {event.title}.</p>
      ...
      <Card className="border-border/40 shadow-xl bg-background/40 backdrop-blur-xl overflow-hidden">
        <CardHeader className="border-b border-border/30 bg-muted/10"><CardTitle>Income Records</CardTitle></CardHeader>
        <CardContent className="p-0">
          {records.length === 0 ? (
            <div className="p-12"><StatePanel icon={SearchSlash} tone="empty" title="No income records" description="No income has been recorded for this event." /></div>
          ) : (
            <Table><TableHeader className="bg-muted/30"><TableRow><TableHead className="font-semibold text-xs uppercase tracking-wider px-6 h-12">Source</TableHead><TableHead className="font-semibold text-xs uppercase tracking-wider h-12">Amount</TableHead><TableHead className="font-semibold text-xs uppercase tracking-wider h-12">Type</TableHead><TableHead className="font-semibold text-xs uppercase tracking-wider h-12">State</TableHead></TableRow></TableHeader>
            <TableBody>{records.map((rec) => (<TableRow key={rec.id} className="hover:bg-muted/20"><TableCell className="px-6 py-4 font-bold">{rec.sourceLabel}</TableCell><TableCell className="font-mono">{formatMoney(rec.amount)}</TableCell><TableCell>{formatEnumLabel(rec.sourceType)}</TableCell><TableCell><Badge variant="neutral" className="text-[10px] uppercase tracking-widest">{formatEnumLabel(rec.state)}</Badge></TableCell></TableRow>))}</TableBody></Table>
          )}
        </CardContent>
      </Card>
    </div>
