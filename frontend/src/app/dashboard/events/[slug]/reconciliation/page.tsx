...
export default async function EventReconciliationPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const reports = await listReconciliationReports({ eventId: event.id }).catch(() => []);
  ...
  return (
    <div className="flex flex-col gap-8 pb-16">
      <section className="relative overflow-hidden rounded-3xl border border-border/20 bg-background/50 shadow-xl backdrop-blur-3xl px-8 py-8">
        ...
        <p className="mt-2 text-muted-foreground text-lg font-light">Closure reports for {event.title}.</p>
        <Badge variant="neutral" className="mt-3">{reports.length} report(s)</Badge>
      </section>
      <Card className="border-border/40 shadow-xl bg-background/40 backdrop-blur-xl overflow-hidden">
        <CardHeader className="border-b border-border/30 bg-muted/10"><CardTitle>Reconciliation Reports</CardTitle></CardHeader>
        <CardContent className="p-0">
          {reports.length === 0 ? (
            <div className="p-12"><StatePanel icon={SearchSlash} tone="empty" title="No reports" description="No reconciliation reports generated for this event yet." /></div>
          ) : (
